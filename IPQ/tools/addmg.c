...
int main(int argc, char* argv[])
{
	size_t cur_size;
	struct stat f_stat;
	int fd, pad_n = 0, append_magic = 0, opt, ret;
	char *pad_buf = NULL;
	...
	if (!append_magic) {
		if((cur_size = f_stat.st_size) > (flash_size - 7)) {
			...
			printf("current boot file size is %u\n", (unsigned int) cur_size);
	...
	lseek(fd, 0, SEEK_END);
	if (pad_n) {
		ret = write(fd, pad_buf, pad_n);
		if (ret != pad_n)
			printf("write pad_buf, %d bytes, return %d\n", pad_n, ret);
	}
	ret = write(fd, magic_buf, sizeof(magic_buf));
	if (ret != sizeof(magic_buf))
		printf("write magic_buf, %lu bytes, return %d\n", sizeof(magic_buf), ret);
	close(fd);
